 ...
 * Author: Paul Reiners
 */
package org.fabrelab.textkit.sequence;
...
public class NeedlemanWunsch extends SequenceAlignment {
	...
	public String getMergedSequence() {
		String[] alignments =  getAlignment();
		String str1 = alignments[0];
		String str2 = alignments[1];
		StringBuffer result = new StringBuffer();
		for(int i=0;i<str1.length();i++){
			if(str1.charAt(i)==str2.charAt(i)){
				if(str1.charAt(i)!='X' || !result.toString().endsWith("XXXXX")){
					result.append(str1.charAt(i));
				}
			}else{
				if(!result.toString().endsWith("XXXXX")){
					result.append("X");
				}
			}
		}
